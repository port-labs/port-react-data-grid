...
const cellRightFrozen = "ch2wcw87-0-0-beta-47";
const cellRightFrozenClassname = `rdg-cell-frozen ${cellRightFrozen}`;
...
function getCellClassname(column, ...extraClasses) {
	return (0, clsx.default)(cellClassname, {
...
function useCalculatedColumns({ rawColumns, defaultColumnOptions, getColumnWidth, viewportWidth, scrollLeft, enableVirtualization }) {
	...
	const { columns, colSpanColumns, lastFrozenColumnIndex, rightFrozenColumnCount, headerRowsCount } = (0, react.useMemo)(() => {
		let lastFrozenColumnIndex$1 = -1;
		let rightFrozenColumnCount$1 = 0;
		...
		function collectColumns(rawColumns$1, level, parent) {
			for (const rawColumn of rawColumns$1) {
				...
				if (rightFrozen) rightFrozenColumnCount$1++;
				if (level > headerRowsCount$1) headerRowsCount$1 = level;
			}
		}
		columns$1.sort(({ key: aKey, frozen: frozenA, rightFrozen: rightFrozenA }, { key: bKey, frozen: frozenB, rightFrozen: rightFrozenB }) => {
		...
		return {
			...
			rightFrozenColumnCount: rightFrozenColumnCount$1,
	...
	const { templateColumns, layoutCssVars, totalFrozenColumnWidth, totalRightFrozenColumnWidth, columnMetrics } = (0, react.useMemo)(() => {
		const columnMetrics$1 = new Map();
		let left = 0;
		let totalFrozenColumnWidth$1 = 0;
		let totalRightFrozenColumnWidth$1 = 0;
		...
		if (rightFrozenColumnCount !== 0) {
			...
			for (let i = columns.length - 1; i >= columns.length - rightFrozenColumnCount; i--) {
				...
				const columnMetric = columnMetrics$1.get(column);
				totalRightFrozenColumnWidth$1 += columnMetric.width;
				layoutCssVars$1[`--rdg-frozen-right-${column.idx}`] = `${rightEnd}px`;
		...
		return {
			...
			totalRightFrozenColumnWidth: totalRightFrozenColumnWidth$1,
	...
	const [colOverscanStartIdx, colOverscanEndIdx] = (0, react.useMemo)(() => {
		...
		const colOverscanEndIdx$1 = min(lastUnfrozonColumnIdx, colVisibleEndIdx + 1);
...
function useViewportColumns({ columns, colSpanColumns, rows, topSummaryRows, bottomSummaryRows, colOverscanStartIdx, colOverscanEndIdx, lastFrozenColumnIndex, rightFrozenColumnCount, rowOverscanStartIdx, rowOverscanEndIdx }) {
...
function DataGrid(props, ref) {
	...
	*/
	...
	const { columns, colSpanColumns, lastFrozenColumnIndex, headerRowsCount, colOverscanStartIdx, colOverscanEndIdx, templateColumns, layoutCssVars, totalFrozenColumnWidth, rightFrozenColumnCount, totalRightFrozenColumnWidth } = useCalculatedColumns({
		rawColumns,
